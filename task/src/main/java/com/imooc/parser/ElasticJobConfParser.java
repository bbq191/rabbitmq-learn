...
import com.imooc.autoconfig.JobZookeeperProperties;
import com.imooc.enums.ElasticJobTypeEnum;
...
public class ElasticJobConfParser implements ApplicationListener<ApplicationReadyEvent> {
  ...
  public void onApplicationEvent(ApplicationReadyEvent event) {
    try {
      ...
      Map<String, Object> beanMap =
          applicationContext.getBeansWithAnnotation(ElasticJobConfig.class);
      for (Object confBean : beanMap.values()) {
        ...
        // 先把当当网的esjob的相关configuration实例化
        ...
        SpringJobScheduler scheduler =
            (SpringJobScheduler) applicationContext.getBean(registerBeanName);
        scheduler.init();
  ...
  private List<BeanDefinition> getTargetElasticJobListeners(ElasticJobConfig conf) {
    ...
    if (StringUtils.hasText(distributedListeners)) {
      BeanDefinitionBuilder factory =
          BeanDefinitionBuilder.rootBeanDefinition(distributedListeners);
      factory.setScope("prototype");
